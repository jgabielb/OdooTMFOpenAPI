...
API_BASE = "/tmf-api/testScenario/v4"
RESOURCE = "testScenario"
PATH = f"{API_BASE}/{RESOURCE}"
ALT_PATH = f"{API_BASE}/TestScenario"
NON_PATCHABLE = {"id", "href"}
MANDATORY_CREATE = ["description", "testScenarioDefinition", "version"]
...
def _find_record(rid):
    model = request.env["tmf.test.scenario"].sudo()
...
class TMF709Controller(http.Controller):
    @http.route([PATH, ALT_PATH], type="http", auth="public", methods=["GET"], csrf=False)
    def list_test_scenario(self, **params):
        model = request.env["tmf.test.scenario"].sudo()
        domain = []
        if params.get("id"):
    ...
    def create_test_scenario(self, **_params):
        data = _parse_json()
        ...
        vals = request.env["tmf.test.scenario"].sudo().from_tmf_json(data)
        rec = request.env["tmf.test.scenario"].sudo().create(vals)
    ...
    def get_test_scenario(self, rid, **params):
    ...
    def patch_test_scenario(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"{RESOURCE} {rid} not found")
        patch = _parse_json()
        ...
        vals = request.env["tmf.test.scenario"].sudo().from_tmf_json(patch, partial=True)
        rec.write(vals)
    ...
    def delete_test_scenario(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"{RESOURCE} {rid} not found")
        rec.unlink()
    ...
    def register_listener(self, **_params):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create(
            {
                "name": f"tmf709-testScenario-{callback}",
    ...
    def unregister_listener(self, sid, **_params):
        ...
        if not rec or not rec.exists() or rec.api_name != "testScenario":
    ...
    @http.route(f"{API_BASE}/listener/testScenarioCreateEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_test_scenario_create(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/testScenarioAttributeValueChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_test_scenario_attribute_change(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/testScenarioStateChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_test_scenario_state_change(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/testScenarioDeleteEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_test_scenario_delete(self, **_params):
        return self._listener_ok()
