...
class TMFGenericResource(models.Model):
    _name = "tmf.self.care.resource"
    _description = "TMF910 Self Care Management Resource"
    ...
    def _get_tmf_api_path(self):
        self.ensure_one()
        return "/tmf-api/tmf-componentsuite/selfcareapp/v1/%s" % (self.resource_type or "troubleTicket")
    ...
    def _sync_native_links(self):
        ...
        for rec in self:
            ...
            try:
                ...
                if not task:
                    vals = {"name": rec.name or f"SelfCare:{rec.tmf_id}", "description": rec.description}
                    if rec.partner_id:
    ...
    def to_tmf_json(self):
        ...
        payload["@type"] = self.tmf_type_value or payload.get("@type") or "TroubleTicket"
        if self.base_type:
