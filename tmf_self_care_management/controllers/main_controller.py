...
RESOURCE_MODEL = "tmf.self.care.resource"
NON_PATCHABLE = {"id", "href"}
RESOURCES = set(('billingAccount','customerBill','appliedCustomerBillingRate','usageConsumptionReport','usage','product','productOffering','productOrder','troubleTicket'))
LISTENER_EVENTS = set(())
...
class TMFController(http.Controller):
    ...
    def register_listener(self, **_params):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create(
            {
                "name": "tmf_self_care_management-" + api_name + "-" + callback,
                "api_name": api_name,
                "callback": callback,
                "query": query,
                "event_type": "any",
    ...
    def openid_userinfo(self, **_params):
        if "tmf_self_care_management" != "tmf_self_care_management":
