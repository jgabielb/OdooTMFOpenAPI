# -*- coding: utf-8 -*-
"""TMFC014 LocationManagement listener + hub controller."""
...
TMFC014_LISTENER_BASE = "/tmfc014/listener"
TMFC014_HUB_BASE = "/tmfc014/hub"
...
class TMFC014ListenerController(http.Controller):
    ...
    @http.route(TMFC014_HUB_BASE,
                type="http", auth="public", methods=["GET", "POST"], csrf=False)
    def hub_register(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc014-")])
        ...
        api_name = data.get("api_name") or "geographicAddress"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc014-{api_name}-{callback}",
            "api_name": api_name,
            "callback": callback,
    ...
    @http.route(f"{TMFC014_HUB_BASE}/<string:sid>",
                type="http", auth="public", methods=["GET", "DELETE"], csrf=False)
    def hub_detail(self, sid, **_p):
        rec = None
        if str(sid).isdigit():
            rec = request.env["tmf.hub.subscription"].sudo().browse(int(sid))
            if not rec.exists() or not rec.name.startswith("tmfc014-"):
                rec = None
        if not rec:
