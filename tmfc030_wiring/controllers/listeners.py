# -*- coding: utf-8 -*-
"""TMFC030 BillGeneration listener + hub controller."""
...
TMFC030_LISTENER_BASE = "/tmfc030/listener"
TMFC030_HUB_BASE = "/tmfc030/hub"

BILLING_ACCOUNT_EVENTS = {
    "BillingAccountCreateEvent", "BillingAccountAttributeValueChangeEvent",
    "BillingAccountStateChangeEvent", "BillingAccountDeleteEvent",
}
PARTY_EVENTS = {
...
class TMFC030ListenerController(http.Controller):
    ...
    def _dispatch(self, payload, allowed, handler_name, source):
        ...
        tools = request.env["tmfc030.wiring.tools"].sudo()
        getattr(tools, handler_name)(payload)
        _logger.info("TMFC030: acknowledged %s event %s", source, ev)
        return self._json_response({}, status=201)

    @http.route(f"{TMFC030_LISTENER_BASE}/billingAccount",
                type="http", auth="public", methods=["POST"], csrf=False)
    def listener_billing_account(self, **_p):
        return self._dispatch(self._parse_json(), BILLING_ACCOUNT_EVENTS, "_handle_billing_account_event", "billingAccount")

    @http.route(f"{TMFC030_LISTENER_BASE}/party",
                type="http", auth="public", methods=["POST"], csrf=False)
    def listener_party(self, **_p):
        return self._dispatch(self._parse_json(), PARTY_EVENTS, "_handle_party_event", "party")

    @http.route(f"{TMFC030_LISTENER_BASE}/partyRole",
                type="http", auth="public", methods=["POST"], csrf=False)
    ...
    @http.route(TMFC030_HUB_BASE,
                type="http", auth="public", methods=["GET", "POST"], csrf=False)
    def hub_register(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc030-")])
        ...
        api_name = data.get("api_name") or "customerBill"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc030-{api_name}-{callback}",
            "api_name": api_name,
            "callback": callback,
    ...
    @http.route(f"{TMFC030_HUB_BASE}/<string:sid>",
                type="http", auth="public", methods=["GET", "DELETE"], csrf=False)
    def hub_detail(self, sid, **_p):
        rec = None
        if str(sid).isdigit():
            rec = request.env["tmf.hub.subscription"].sudo().browse(int(sid))
            if not rec.exists() or not rec.name.startswith("tmfc030-"):
                rec = None
        if not rec:
