...
class TMFC041ListenerController(http.Controller):
    ...
    def _dispatch(self, payload, allowed, handler):
        ...
        getattr(request.env["tmfc041.wiring.tools"].sudo(), handler)(payload)
    ...
    def hub(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc041-")])
        ...
        api_name = data.get("api_name") or "incident"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc041-{api_name}-{cb}", "api_name": api_name, "callback": cb,
    ...
    def hub_detail(self, sid, **_p):
        ...
        if str(sid).isdigit():
            ...
            if not rec.exists() or not rec.name.startswith("tmfc041-"):
                rec = None
        if not rec:
