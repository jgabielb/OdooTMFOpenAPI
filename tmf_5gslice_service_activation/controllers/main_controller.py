...
class TMF5gsliceServiceActivationController(TMFBaseController):
    ...
    def service_collection(self, **kw):
        if request.httprequest.method == "POST":
            return self._tmf_create("service")
        return self._tmf_list("service", **kw)
    ...
    def service_individual(self, rid, **kw):
        return self._tmf_individual("service", rid, **kw)
    @http.route(
        [RESOURCES["monitor"]["path"]],
        type="http", auth="public", methods=["GET", "POST"], csrf=False)
    def monitor_collection(self, **kw):
        if request.httprequest.method == "POST":
            return self._tmf_create("monitor")
        return self._tmf_list("monitor", **kw)

    @http.route(
        [RESOURCES["monitor"]["path"] + "/<string:rid>"],
        type="http", auth="public", methods=["GET", "PATCH", "DELETE"], csrf=False)
    def monitor_individual(self, rid, **kw):
        return self._tmf_individual("monitor", rid, **kw)
