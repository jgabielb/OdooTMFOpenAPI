...
class TMF5gsliceServiceActivationController(TMFBaseController):
    ...
    def hub(self, **_kw):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmf_5gslice_service_activation-{callback}",
            "api_name": "service",
            "callback": callback,
    ...
    def service_collection(self, **kw):
        if request.httprequest.method == "POST":
            return self._tmf_do_create(RESOURCES["service"])
        return self._tmf_do_list(RESOURCES["service"], **kw)

    @http.route(
        [RESOURCES["service"]["path"] + "/<string:rid>"],
        type="http", auth="public", methods=["GET", "PATCH", "DELETE"], csrf=False)
    def service_individual(self, rid, **kw):
        return self._tmf_do_individual(RESOURCES["service"], rid, **kw)
    @http.route(
        [RESOURCES["monitor"]["path"]],
        type="http", auth="public", methods=["GET", "POST"], csrf=False)
    def monitor_collection(self, **kw):
        if request.httprequest.method == "POST":
            return self._tmf_do_create(RESOURCES["monitor"])
        return self._tmf_do_list(RESOURCES["monitor"], **kw)

    @http.route(
        [RESOURCES["monitor"]["path"] + "/<string:rid>"],
        type="http", auth="public", methods=["GET", "PATCH", "DELETE"], csrf=False)
    def monitor_individual(self, rid, **kw):
        return self._tmf_do_individual(RESOURCES["monitor"], rid, **kw)

    @http.route(f"{API_BASE}/listener/ServiceCreateEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listener_servicecreateevent(self, **_kw):
        return self._listener_ack()
    @http.route(f"{API_BASE}/listener/ServiceAttributeValueChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listener_serviceattributevaluechangeevent(self, **_kw):
        return self._listener_ack()
    @http.route(f"{API_BASE}/listener/ServiceStateChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listener_servicestatechangeevent(self, **_kw):
        return self._listener_ack()
    @http.route(f"{API_BASE}/listener/ServiceDeleteEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listener_servicedeleteevent(self, **_kw):
        return self._listener_ack()
    @http.route(f"{API_BASE}/listener/MonitorCreateEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listener_monitorcreateevent(self, **_kw):
        return self._listener_ack()
    @http.route(f"{API_BASE}/listener/MonitorAttributeValueChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listener_monitorattributevaluechangeevent(self, **_kw):
        return self._listener_ack()
    @http.route(f"{API_BASE}/listener/MonitorStateChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listener_monitorstatechangeevent(self, **_kw):
        return self._listener_ack()
    @http.route(f"{API_BASE}/listener/MonitorDeleteEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listener_monitordeleteevent(self, **_kw):
        return self._listener_ack()
