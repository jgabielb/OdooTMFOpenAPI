...
RESOURCE_MODEL = "tmf.5gslice.service.activation.resource"
NON_PATCHABLE = {"id", "href"}
RESOURCES = set(('service','monitor'))
...
class TMFController(http.Controller):
    ...
    def register_listener(self, **_params):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create(
            {
                "name": "tmf_5gslice_service_activation-" + api_name + "-" + callback,
                "api_name": api_name,
                "callback": callback,
                "query": query,
                "event_type": "any",
    ...
    def openid_userinfo(self, **_params):
        if "tmf_5gslice_service_activation" != "tmf_self_care_management":
