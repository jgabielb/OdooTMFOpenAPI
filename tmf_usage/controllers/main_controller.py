...
class TMFController(http.Controller):
    ...
    def get_resources(self, **params):
        records = request.env['tmf.usage'].sudo().search([])
    ...
    def create_resource(self, **params):
        try:
            ...
            vals = {}
            # Basic field mapping
            for field in ['description', 'usage_date', 'usage_type', 'rated_product_usage', 'related_party', 'status', 'usage_characteristic', 'usage_specification']:
                if field in data:
                    vals[field] = data[field]
            
            new_rec = request.env['tmf.usage'].sudo().create(vals)
            return request.make_response(
