...
API_BASE = "/tmf-api/generalTestArtifact/v4"
RESOURCE = "generalTestArtifact"
PATH = f"{API_BASE}/{RESOURCE}"
ALT_PATH = f"{API_BASE}/GeneralTestArtifact"
NON_PATCHABLE = {"id", "href"}
MANDATORY_CREATE = ["description", "version"]
...
def _find_record(rid):
    model = request.env["tmf.general.test.artifact"].sudo()
...
class TMF710Controller(http.Controller):
    @http.route([PATH, ALT_PATH], type="http", auth="public", methods=["GET"], csrf=False)
    def list_general_test_artifact(self, **params):
        model = request.env["tmf.general.test.artifact"].sudo()
        domain = []
        if params.get("id"):
    ...
    def create_general_test_artifact(self, **_params):
        data = _parse_json()
        ...
        vals = request.env["tmf.general.test.artifact"].sudo().from_tmf_json(data)
        rec = request.env["tmf.general.test.artifact"].sudo().create(vals)
    ...
    def get_general_test_artifact(self, rid, **params):
    ...
    def patch_general_test_artifact(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"{RESOURCE} {rid} not found")
        patch = _parse_json()
        ...
        vals = request.env["tmf.general.test.artifact"].sudo().from_tmf_json(patch, partial=True)
        rec.write(vals)
    ...
    def delete_general_test_artifact(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"{RESOURCE} {rid} not found")
        rec.unlink()
    ...
    def register_listener(self, **_params):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create(
            {
                "name": f"tmf710-generalTestArtifact-{callback}",
    ...
    def unregister_listener(self, sid, **_params):
        ...
        if not rec or not rec.exists() or rec.api_name != "generalTestArtifact":
    ...
    @http.route(f"{API_BASE}/listener/generalTestArtifactCreateEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_general_test_artifact_create(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/generalTestArtifactAttributeValueChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_general_test_artifact_attribute_change(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/generalTestArtifactStateChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_general_test_artifact_state_change(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/generalTestArtifactDeleteEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_general_test_artifact_delete(self, **_params):
        return self._listener_ok()
