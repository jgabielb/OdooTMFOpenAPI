...
class TMFGenericResource(models.Model):
    _name = "tmf.naas.resource"
    _description = "TMF909 Network as a Service Management Resource"
    ...
    def _get_tmf_api_path(self):
        self.ensure_one()
        return "/tmf-api/naas/v4/%s" % (self.resource_type or "service")
    ...
    def _resolve_sale_order(self):
        self.ensure_one()
        payload = _loads(self.payload_json)
        order_ref = payload.get("order") or payload.get("serviceOrder") or payload.get("productOrder") or {}
        if not isinstance(order_ref, dict):
            return False
        rid = order_ref.get("id")
        if not rid:
            return False
        return self.env["sale.order"].sudo().search([("client_order_ref", "=", str(rid))], limit=1)
