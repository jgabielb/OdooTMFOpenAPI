...
API_BASE = "/tmf-api/digitalIdentityManagement/v4"
PATH = f"{API_BASE}/digitalIdentity"
ALT_PATH = f"{API_BASE}/DigitalIdentity"
NON_PATCHABLE = {"id", "href"}
MANDATORY_CREATE = ["credential"]
...
def _find_record(rid):
    model = request.env["tmf.digital.identity"].sudo()
...
class TMF720Controller(http.Controller):
    @http.route([PATH, ALT_PATH], type="http", auth="public", methods=["GET"], csrf=False)
    def list_digital_identity(self, **params):
        model = request.env["tmf.digital.identity"].sudo()
        domain = []
        if params.get("id"):
    ...
    def create_digital_identity(self, **_params):
        data = _parse_json()
        ...
        vals = request.env["tmf.digital.identity"].sudo().from_tmf_json(data)
        rec = request.env["tmf.digital.identity"].sudo().create(vals)
    ...
    def get_digital_identity(self, rid, **params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"digitalIdentity {rid} not found")
    ...
    def patch_digital_identity(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"digitalIdentity {rid} not found")
        patch = _parse_json()
        ...
        vals = request.env["tmf.digital.identity"].sudo().from_tmf_json(patch, partial=True)
        rec.write(vals)
    ...
    def delete_digital_identity(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"digitalIdentity {rid} not found")
        rec.unlink()
    ...
    def register_listener(self, **_params):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create(
            {
                "name": f"tmf720-digitalIdentity-{callback}",
    ...
    def unregister_listener(self, sid, **_params):
        ...
        if not rec or not rec.exists() or rec.api_name != "digitalIdentity":
    ...
    @http.route(f"{API_BASE}/listener/digitalIdentityCreateEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_digital_identity_create(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/digitalIdentityAttributeValueChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_digital_identity_change(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/digitalIdentityStateChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_digital_identity_state(self, **_params):
        return self._listener_ok()
