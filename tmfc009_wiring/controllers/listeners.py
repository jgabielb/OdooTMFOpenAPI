# -*- coding: utf-8 -*-
"""TMFC009 ServiceQualificationManagement listener + hub controller."""
...
TMFC009_LISTENER_BASE = "/tmfc009/listener"
TMFC009_HUB_BASE = "/tmfc009/hub"
...
class TMFC009ListenerController(http.Controller):
    ...
    @http.route(f"{TMFC009_LISTENER_BASE}/serviceSpecification",
                type="http", auth="public", methods=["POST"], csrf=False)
    def listener_service_spec(self, **_p):
        return self._dispatch(self._parse_json(), SERVICE_SPEC_EVENTS, "_handle_service_specification_event", "serviceSpecification")

    @http.route(TMFC009_HUB_BASE,
                type="http", auth="public", methods=["GET", "POST"], csrf=False)
    def hub_register(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc009-")])
        ...
        api_name = data.get("api_name") or "serviceQualification"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc009-{api_name}-{callback}",
            "api_name": api_name,
            "callback": callback,
    ...
    @http.route(f"{TMFC009_HUB_BASE}/<string:sid>",
                type="http", auth="public", methods=["GET", "DELETE"], csrf=False)
    def hub_detail(self, sid, **_p):
        rec = None
        if str(sid).isdigit():
            rec = request.env["tmf.hub.subscription"].sudo().browse(int(sid))
            if not rec.exists() or not rec.name.startswith("tmfc009-"):
                rec = None
        if not rec:
