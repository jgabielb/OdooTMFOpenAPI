...
class TMFC061WorkOrderManagement(models.Model):
    _inherit = "tmf.work"

    tmfc061_related_partner_ids = fields.Many2many(
        "res.partner", "tmfc061_partner_rel", "rec_id", "partner_id",
        string="TMFC061 Related Partners",
    )
    tmfc061_party_role_ids = fields.Many2many(
        "tmf.party.role", "tmfc061_party_role_rel", "rec_id", "party_role_id",
        string="TMFC061 Party Roles",
    )

    def _tmfc061_resolve_refs(self):
        ...
        for rec in self:
            ...
            if party_refs:
                ...
                if p:
                    updates["tmfc061_related_partner_ids"] = [(6, 0, p.ids)]
            if role_refs:
                r = PartyRole.search([("tmf_id", "in", role_refs)])
                if r:
                    updates["tmfc061_party_role_ids"] = [(6, 0, r.ids)]
            if updates:
    ...
    def write(self, vals):
        ...
        if not self.env.context.get("skip_tmf_wiring") and "related_party_json" in vals:
            try:
                self._tmfc061_resolve_refs()
            except Exception:
                pass
        return res


class TMFC061WiringTools(models.AbstractModel):
    _name = "tmfc061.wiring.tools"
    _description = "TMFC061 Wiring Tools"

    @api.model
    def _handle_party_event(self, payload):
        Rec = self.env["tmf.work"].sudo()
        Rec.search([])._tmfc061_resolve_refs()
        return True
    ...
    @api.model
    def _handle_appointment_event(self, payload):
        """TMF646 appointment/searchTimeSlot events: sync local appointment state."""
        resource = payload or {}
        event = resource.get("event")
        if isinstance(event, dict):
            for value in event.values():
        ...
        if not ref_id:
            return False
        rec = self.env["tmf.appointment"].sudo().search(
