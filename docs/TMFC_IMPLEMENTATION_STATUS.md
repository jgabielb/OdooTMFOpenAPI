...
| TMFC005  | ProductInventory                               | ⚠️ | `tmfc005_wiring` exists and links `tmf.product` to Odoo `product.template`, `product.product`, `stock.location`, `stock.lot`, and `stock.quant`. `tmf_product_inventory` extends `tmf.product` with TMF637 productInventory fields and publishes ProductCreate/AttributeValueChange/Delete events via `tmf.hub.subscription`. TMFC005 remains partially wired because TMF701 dependencies, subscribed-event handling, and full YAML dependency coverage have not yet been verified. |
