...
API_BASE = "/tmf-api/privacyManagement/v5"
RESOURCE_MODEL = "tmf.privacy.management.resource"
...
DEFAULT_TYPES = {
    "partyPrivacyProfile": "PartyPrivacyProfile",
    "partyPrivacyProfileSpecification": "PartyPrivacyProfileSpecification",
    "partyPrivacyAgreement": "PartyPrivacyAgreement",
}

RESOURCE_ALIAS = {name.lower(): name for name in RESOURCES}
TYPE_TO_RESOURCE = {
    "partyprivacyprofile": "partyPrivacyProfile",
    "partyprivacyprofilespecification": "partyPrivacyProfileSpecification",
    "partyprivacyagreement": "partyPrivacyAgreement",
...
def _resolve_resource(resource):
    if not resource:
        return None
    return RESOURCE_ALIAS.get(str(resource).strip("/").lower())


def _infer_resource(resource, data=None, params=None):
    resolved = _resolve_resource(resource)
    if resolved:
        return resolved

    params = params or {}
    for key in ("resource", "resourceType", "type"):
        candidate = _resolve_resource(params.get(key))
        if candidate:
            return candidate
    ...
    if isinstance(data, dict):
        ...
        if isinstance(tval, str):
            candidate = TYPE_TO_RESOURCE.get(tval.lower())
            if candidate:
                return candidate
        # Heuristic for profile payload shape.
        if "agreedByParty" in data or "partyPrivacyProfileCharacteristic" in data:
            return "partyPrivacyProfile"
        if "partyPrivacyProfileSpecification" in data or "partyPrivacyProfileSpecCharacteristic" in data:
            return "partyPrivacyProfileSpecification"
        if "partyPrivacyProfile" in data:
            return "partyPrivacyAgreement"
    return None


class TMF644Controller(http.Controller):
    @http.route([API_BASE, API_BASE + "/", API_BASE + "/<string:resource>", API_BASE + "/<string:resource>/"], type="http", auth="public", methods=["GET"], csrf=False)
    def list_resource(self, resource=None, **params):
        resource = _infer_resource(resource, params=params)
        if resource not in RESOURCES:
            return _error(400, "Missing or invalid resource path")
        model = request.env[RESOURCE_MODEL].sudo()
    ...
    @http.route([API_BASE, API_BASE + "/", API_BASE + "/<string:resource>", API_BASE + "/<string:resource>/"], type="http", auth="public", methods=["POST"], csrf=False)
    def create_resource(self, resource=None, **params):
        data = _parse_json()
        resource = _infer_resource(resource, data=data, params=params)
        if resource not in RESOURCES:
            return _error(400, "Missing or invalid resource path")
        if not isinstance(data, dict):
            return _error(400, "Invalid JSON body")
        data.setdefault("@type", DEFAULT_TYPES.get(resource, resource[:1].upper() + resource[1:]))
    ...
    @http.route([API_BASE + "/<string:resource>/<string:rid>", API_BASE + "/<string:resource>/<string:rid>/"], type="http", auth="public", methods=["GET"], csrf=False)
    ...
    @http.route([API_BASE + "/<string:resource>/<string:rid>", API_BASE + "/<string:resource>/<string:rid>/"], type="http", auth="public", methods=["PATCH"], csrf=False)
    def patch_resource(self, resource, rid, **_params):
        resource = _infer_resource(resource)
        if resource not in RESOURCES:
            return _error(404, f"Resource {resource} not found")
        rec = _find_record(resource, rid)
        if not rec:
    ...
    @http.route([API_BASE + "/<string:resource>/<string:rid>", API_BASE + "/<string:resource>/<string:rid>/"], type="http", auth="public", methods=["DELETE"], csrf=False)
    ...
    def register_listener(self, **_params):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create(
            {
                "name": f"tmf644-{api_name}-{callback}",
                "api_name": api_name,
                "callback": callback,
                "query": query,
                "event_type": "any",
