...
class TMFGenericResource(models.Model):
    _name = "tmf.intent.management.resource"
    _description = "TMF921 Intent Management Resource"
    ...
    parent_intent_id = fields.Char(string="parentIntentId", index=True)
    ...
    def _get_tmf_api_path(self):
        self.ensure_one()
        if self.resource_type == "intentReport" and self.parent_intent_id:
            return "/tmf-api/intentManagement/v5/intent/%s/intentReport" % self.parent_intent_id
        return "/tmf-api/intentManagement/v5/%s" % (self.resource_type or "intent")
    ...
    def to_tmf_json(self):
        ...
        payload["@type"] = self.tmf_type_value or payload.get("@type") or default_types.get(self.resource_type, "Intent")
        if self.base_type:
    ...
    def from_tmf_json(self, data, resource_type=None, partial=False):
        ...
        if "parent_intent_id" in data:
            vals["parent_intent_id"] = data.get("parent_intent_id")
        return vals
