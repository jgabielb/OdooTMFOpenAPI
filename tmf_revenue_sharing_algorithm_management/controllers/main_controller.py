...
API_BASE = "/tmf-api/revenueSharingAlgorithmManagement/v5"
PATH = f"{API_BASE}/partyRevSharingAlgorithm"
ALT_PATH = f"{API_BASE}/PartyRevSharingAlgorithm"
NON_PATCHABLE = {"id", "href"}
MANDATORY_CREATE = ["@type", "name"]
...
def _find_record(rid):
    model = request.env["tmf.party.rev.sharing.algorithm"].sudo()
...
class TMF736Controller(http.Controller):
    @http.route([PATH, ALT_PATH], type="http", auth="public", methods=["GET"], csrf=False)
    def list_party_rev_sharing_algorithm(self, **params):
        model = request.env["tmf.party.rev.sharing.algorithm"].sudo()
        domain = []
        if params.get("id"):
    ...
    def create_party_rev_sharing_algorithm(self, **_params):
        data = _parse_json()
        ...
        vals = request.env["tmf.party.rev.sharing.algorithm"].sudo().from_tmf_json(data)
        rec = request.env["tmf.party.rev.sharing.algorithm"].sudo().create(vals)
    ...
    def get_party_rev_sharing_algorithm(self, rid, **params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"partyRevSharingAlgorithm {rid} not found")
    ...
    def patch_party_rev_sharing_algorithm(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"partyRevSharingAlgorithm {rid} not found")
        patch = _parse_json()
        ...
        vals = request.env["tmf.party.rev.sharing.algorithm"].sudo().from_tmf_json(patch, partial=True)
        rec.write(vals)
    ...
    def delete_party_rev_sharing_algorithm(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"partyRevSharingAlgorithm {rid} not found")
        rec.unlink()
    ...
    def register_listener(self, **_params):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create(
            {
                "name": f"tmf736-partyRevSharingAlgorithm-{callback}",
    ...
    def unregister_listener(self, sid, **_params):
        ...
        if not rec or not rec.exists() or rec.api_name != "partyRevSharingAlgorithm":
    ...
    @http.route(f"{API_BASE}/listener/partyRevSharingAlgorithmCreateEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_party_rev_sharing_algorithm_create(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/partyRevSharingAlgorithmAttributeValueChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_party_rev_sharing_algorithm_change(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/partyRevSharingAlgorithmStateChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_party_rev_sharing_algorithm_state(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/partyRevSharingAlgorithmDeleteEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_party_rev_sharing_algorithm_delete(self, **_params):
        return self._listener_ok()
