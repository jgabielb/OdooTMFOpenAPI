...
class TMFController(http.Controller):
    ...
    def get_resources(self, **params):
        records = request.env['tmf.geographic.location'].sudo().search([])
        return request.make_response(
            json.dumps([r.to_tmf_json() for r in records]),
            headers=[('Content-Type', 'application/json')]
        )

    @http.route('/tmf-api/geographicLocationManagement/v4/GeographicLocation', type='http', auth='public', methods=['POST'], csrf=False)
    def create_resource(self, **params):
        try:
            ...
            new_rec = request.env['tmf.geographic.location'].sudo().create(vals)
            return request.make_response(
