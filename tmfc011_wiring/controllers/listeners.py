# -*- coding: utf-8 -*-
"""TMFC011 ResourceOrderManagement listener + hub controller."""
...
TMFC011_LISTENER_BASE = "/tmfc011/listener"
TMFC011_HUB_BASE = "/tmfc011/hub"
...
class TMFC011ListenerController(http.Controller):
    ...
    @http.route(f"{TMFC011_LISTENER_BASE}/resourceSpecification",
                type="http", auth="public", methods=["POST"], csrf=False)
    ...
    def hub_register(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc011-")])
        ...
        api_name = data.get("api_name") or "resourceOrder"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc011-{api_name}-{callback}",
            "api_name": api_name,
            "callback": callback,
    ...
    @http.route(f"{TMFC011_HUB_BASE}/<string:sid>",
                type="http", auth="public", methods=["GET", "DELETE"], csrf=False)
    def hub_detail(self, sid, **_p):
        rec = None
        if str(sid).isdigit():
            rec = request.env["tmf.hub.subscription"].sudo().browse(int(sid))
            if not rec.exists() or not rec.name.startswith("tmfc011-"):
                rec = None
        if not rec:
