...
API_BASE = "/tmf-api/cdrTransactionManagement/v5"
PATH = f"{API_BASE}/cdrTransaction"
ALT_PATH = f"{API_BASE}/CdrTransaction"
NON_PATCHABLE = {"id", "href"}
MANDATORY_CREATE = ["relatedParty", "channel", "@type"]
...
def _find_record(rid):
    model = request.env["tmf.cdr.transaction"].sudo()
...
class TMF735Controller(http.Controller):
    @http.route([PATH, ALT_PATH], type="http", auth="public", methods=["GET"], csrf=False)
    def list_cdr_transaction(self, **params):
        model = request.env["tmf.cdr.transaction"].sudo()
        domain = []
        if params.get("id"):
        ...
        if params.get("requestedInitialState"):
            domain.append(("requested_initial_state", "=", params["requestedInitialState"]))
    ...
    def create_cdr_transaction(self, **_params):
        data = _parse_json()
        ...
        vals = request.env["tmf.cdr.transaction"].sudo().from_tmf_json(data)
        rec = request.env["tmf.cdr.transaction"].sudo().create(vals)
    ...
    def get_cdr_transaction(self, rid, **params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"cdrTransaction {rid} not found")
    ...
    def patch_cdr_transaction(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"cdrTransaction {rid} not found")
        patch = _parse_json()
        ...
        vals = request.env["tmf.cdr.transaction"].sudo().from_tmf_json(patch, partial=True)
        rec.write(vals)
    ...
    def delete_cdr_transaction(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"cdrTransaction {rid} not found")
        rec.unlink()
    ...
    def register_listener(self, **_params):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create(
            {
                "name": f"tmf735-cdrTransaction-{callback}",
    ...
    def unregister_listener(self, sid, **_params):
        ...
        if not rec or not rec.exists() or rec.api_name != "cdrTransaction":
    ...
    @http.route(f"{API_BASE}/listener/cdrTransactionCreateEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_cdr_transaction_create(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/cdrTransactionAttributeValueChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_cdr_transaction_change(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/cdrTransactionStateChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_cdr_transaction_state(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/cdrTransactionDeleteEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_cdr_transaction_delete(self, **_params):
        return self._listener_ok()
