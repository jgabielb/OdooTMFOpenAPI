...
SERVICE_TEST_EVENTS = {
    ...
    "ServiceTestSpecificationChangeEvent", "ServiceTestSpecificationDeleteEvent",
}


class TMFC054ListenerController(http.Controller):
    def _parse(self):
    ...
    def _dispatch(self, payload, allowed, handler):
        ...
        getattr(request.env["tmfc054.wiring.tools"].sudo(), handler)(payload)
    ...
    @http.route(f"{BASE_L}/serviceTest", type="http", auth="public", methods=["POST"], csrf=False)
    def l_service_test(self, **_p):
        return self._dispatch(self._parse(), SERVICE_TEST_EVENTS, "_handle_service_test_event")

    @http.route(BASE_H, type="http", auth="public", methods=["GET", "POST"], csrf=False)
    def hub(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc054-")])
        ...
        api_name = data.get("api_name") or "testCase"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc054-{api_name}-{cb}", "api_name": api_name, "callback": cb,
    ...
    def hub_detail(self, sid, **_p):
        ...
        if str(sid).isdigit():
            ...
            if not rec.exists() or not rec.name.startswith("tmfc054-"):
                rec = None
        if not rec:
