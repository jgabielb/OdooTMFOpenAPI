...
PATH = f"{API_BASE}/serviceUsage"
ALT_PATH = f"{API_BASE}/ServiceUsage"
NON_PATCHABLE = {"id", "href"}
MANDATORY_CREATE = ["status"]
...
def _find_record(rid):
    model = request.env["tmf.service.usage"].sudo()
...
class TMF727Controller(http.Controller):
    @http.route([PATH, ALT_PATH], type="http", auth="public", methods=["GET"], csrf=False)
    def list_service_usage(self, **params):
        model = request.env["tmf.service.usage"].sudo()
        domain = []
        if params.get("id"):
        ...
        if params.get("usageDate"):
            domain.append(("usage_date", "=", params["usageDate"]))
    ...
    def create_service_usage(self, **_params):
        data = _parse_json()
        ...
        vals = request.env["tmf.service.usage"].sudo().from_tmf_json(data)
        rec = request.env["tmf.service.usage"].sudo().create(vals)
    ...
    def get_service_usage(self, rid, **params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"serviceUsage {rid} not found")
    ...
    def patch_service_usage(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"serviceUsage {rid} not found")
        patch = _parse_json()
        ...
        vals = request.env["tmf.service.usage"].sudo().from_tmf_json(patch, partial=True)
        rec.write(vals)
    ...
    def delete_service_usage(self, rid, **_params):
        rec = _find_record(rid)
        if not rec:
            return _error(404, f"serviceUsage {rid} not found")
        rec.unlink()
    ...
    def register_listener(self, **_params):
        ...
        rec = request.env["tmf.hub.subscription"].sudo().create(
            {
                "name": f"tmf727-serviceUsage-{callback}",
    ...
    def unregister_listener(self, sid, **_params):
        ...
        if not rec or not rec.exists() or rec.api_name != "serviceUsage":
    ...
    def listen_service_usage_create(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/serviceUsageStatusChangeEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_service_usage_status_change(self, **_params):
        return self._listener_ok()

    @http.route(f"{API_BASE}/listener/serviceUsageDeleteEvent", type="http", auth="public", methods=["POST"], csrf=False)
    def listen_service_usage_delete(self, **_params):
        return self._listener_ok()
