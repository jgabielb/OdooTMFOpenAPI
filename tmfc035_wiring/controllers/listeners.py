# -*- coding: utf-8 -*-
"""TMFC035 PermissionsManagement listener + hub controller."""
...
TMFC035_LISTENER_BASE = "/tmfc035/listener"
TMFC035_HUB_BASE = "/tmfc035/hub"
...
class TMFC035ListenerController(http.Controller):
    ...
    def hub_register(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc035-")])
        ...
        api_name = data.get("api_name") or "permission"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc035-{api_name}-{callback}",
            "api_name": api_name,
            "callback": callback,
    ...
    @http.route(f"{TMFC035_HUB_BASE}/<string:sid>",
                type="http", auth="public", methods=["GET", "DELETE"], csrf=False)
    def hub_detail(self, sid, **_p):
        rec = None
        if str(sid).isdigit():
            rec = request.env["tmf.hub.subscription"].sudo().browse(int(sid))
            if not rec.exists() or not rec.name.startswith("tmfc035-"):
                rec = None
        if not rec:
