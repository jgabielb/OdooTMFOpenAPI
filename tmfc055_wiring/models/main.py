...
class TMFC055ServiceTestManagement(models.Model):
    _inherit = "tmf.service.test"

    tmfc055_related_partner_ids = fields.Many2many(
        "res.partner", "tmfc055_partner_rel", "rec_id", "partner_id",
        string="TMFC055 Related Partners",
    )
    tmfc055_party_role_ids = fields.Many2many(
        "tmf.party.role", "tmfc055_party_role_rel", "rec_id", "party_role_id",
        string="TMFC055 Party Roles",
    )

    def _tmfc055_resolve_refs(self):
        ...
        for rec in self:
            ...
            if party_refs:
                ...
                if p:
                    updates["tmfc055_related_partner_ids"] = [(6, 0, p.ids)]
            if role_refs:
                r = PartyRole.search([("tmf_id", "in", role_refs)])
                if r:
                    updates["tmfc055_party_role_ids"] = [(6, 0, r.ids)]
            if updates:
    ...
    def write(self, vals):
        ...
        if not self.env.context.get("skip_tmf_wiring") and "related_party_json" in vals:
            try:
                self._tmfc055_resolve_refs()
            except Exception:
                pass
        return res


class TMFC055WiringTools(models.AbstractModel):
    _name = "tmfc055.wiring.tools"
    _description = "TMFC055 Wiring Tools"
