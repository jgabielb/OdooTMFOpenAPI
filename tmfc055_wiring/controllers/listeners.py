...
class TMFC055ListenerController(http.Controller):
    ...
    def _dispatch(self, payload, allowed, handler):
        ...
        getattr(request.env["tmfc055.wiring.tools"].sudo(), handler)(payload)
    ...
    def hub(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc055-")])
        ...
        api_name = data.get("api_name") or "serviceTest"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc055-{api_name}-{cb}", "api_name": api_name, "callback": cb,
    ...
    def hub_detail(self, sid, **_p):
        ...
        if str(sid).isdigit():
            ...
            if not rec.exists() or not rec.name.startswith("tmfc055-"):
                rec = None
        if not rec:
