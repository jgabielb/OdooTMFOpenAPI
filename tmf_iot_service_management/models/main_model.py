...
class TMFGenericResource(models.Model):
    _name = "tmf.iot.service.resource"
    _description = "TMF914 IoT Service Management Resource"
    ...
    def _get_tmf_api_path(self):
        self.ensure_one()
        return "/tmf-api/iotservicemanagement/v4/%s" % (self.resource_type or "iotService")
    ...
    def _resolve_sale_order(self):
        self.ensure_one()
        payload = _loads(self.payload_json)
        order_ref = payload.get("order") or payload.get("productOrder") or {}
        if not isinstance(order_ref, dict):
            return False
        rid = order_ref.get("id")
        if not rid:
            return False
        env_so = self.env["sale.order"].sudo()
        return env_so.search([("client_order_ref", "=", str(rid))], limit=1)
    ...
    def to_tmf_json(self):
        ...
        payload["@type"] = self.tmf_type_value or payload.get("@type") or "IotService"
        if self.base_type:
    ...
    def write(self, vals):
        ...
        if "payload_json" in vals or "partner_id" in vals or "sale_order_id" in vals:
