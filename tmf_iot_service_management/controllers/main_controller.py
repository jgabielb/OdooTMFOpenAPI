...
RESOURCES = {
    "iotService": {"model": "tmf.iot.service.resource", "path": f"{API_BASE}/iotService", "required": []},
    "iotServiceSpecification": {"model": "tmf.iot.service.resource", "path": f"{API_BASE}/iotServiceSpecification", "required": []},
}
...
class TMFIotServiceManagementController(TMFBaseController):
    ...
    def iotService_collection(self, **kw):
        if request.httprequest.method == "POST":
            return self._tmf_create("iotService")
        return self._tmf_list("iotService", **kw)
    ...
    def iotService_individual(self, rid, **kw):
        return self._tmf_individual("iotService", rid, **kw)
    @http.route(
        [RESOURCES["iotServiceSpecification"]["path"]],
        type="http", auth="public", methods=["GET", "POST"], csrf=False)
    def iotServiceSpecification_collection(self, **kw):
        if request.httprequest.method == "POST":
            return self._tmf_create("iotServiceSpecification")
        return self._tmf_list("iotServiceSpecification", **kw)

    @http.route(
        [RESOURCES["iotServiceSpecification"]["path"] + "/<string:rid>"],
        type="http", auth="public", methods=["GET", "PATCH", "DELETE"], csrf=False)
    def iotServiceSpecification_individual(self, rid, **kw):
        return self._tmf_individual("iotServiceSpecification", rid, **kw)
