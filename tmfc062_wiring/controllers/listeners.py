...
class TMFC062ListenerController(http.Controller):
    def _parse(self):
    ...
    def _dispatch(self, payload, allowed, handler):
        ...
        getattr(request.env["tmfc062.wiring.tools"].sudo(), handler)(payload)
    ...
    def hub(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc062-")])
        ...
        api_name = data.get("api_name") or "resourceFunction"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc062-{api_name}-{cb}", "api_name": api_name, "callback": cb,
    ...
    def hub_detail(self, sid, **_p):
        ...
        if str(sid).isdigit():
            ...
            if not rec.exists() or not rec.name.startswith("tmfc062-"):
                rec = None
        if not rec:
