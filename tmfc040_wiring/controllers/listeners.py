# -*- coding: utf-8 -*-
"""TMFC040 UsageManagement listener + hub controller."""
...
TMFC040_LISTENER_BASE = "/tmfc040/listener"
TMFC040_HUB_BASE = "/tmfc040/hub"
...
class TMFC040ListenerController(http.Controller):
    ...
    @http.route(f"{TMFC040_LISTENER_BASE}/billingAccount",
                type="http", auth="public", methods=["POST"], csrf=False)
    ...
    @http.route(TMFC040_HUB_BASE,
                type="http", auth="public", methods=["GET", "POST"], csrf=False)
    def hub_register(self, **_p):
        if request.httprequest.method == "GET":
            subs = request.env["tmf.hub.subscription"].sudo().search([("name", "like", "tmfc040-")])
        ...
        api_name = data.get("api_name") or "usage"
        rec = request.env["tmf.hub.subscription"].sudo().create({
            "name": f"tmfc040-{api_name}-{callback}",
            "api_name": api_name,
            "callback": callback,
    ...
    @http.route(f"{TMFC040_HUB_BASE}/<string:sid>",
                type="http", auth="public", methods=["GET", "DELETE"], csrf=False)
    def hub_detail(self, sid, **_p):
        rec = None
        if str(sid).isdigit():
            rec = request.env["tmf.hub.subscription"].sudo().browse(int(sid))
            if not rec.exists() or not rec.name.startswith("tmfc040-"):
                rec = None
        if not rec:
